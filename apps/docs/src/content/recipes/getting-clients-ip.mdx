...
Older versions of Kaito were built on top of Node.js only, so you could access `req.socket.remoteAddress` directly. Since Kaito now uses the standard Web Fetch API, there's no `.socket` property on requests — server runtimes provide their own way to access the remote address.
