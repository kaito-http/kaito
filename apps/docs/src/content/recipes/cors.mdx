...
`experimental_createCORSTransform()` gives you an object with `.before` and `.transform` that you can pass straight into your router config. It also lets you manage origins dynamically at runtime:

- `cors.addOrigins('https://new.example.com')` — add new allowed origins
- `cors.removeOrigins('https://old.example.com')` — remove origins
- `cors.setOrigins(['https://only-this.example.com'])` — replace all origins
- `cors.getOrigins()` — see what's currently allowed
