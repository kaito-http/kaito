---
title: Handling CORS with Kaito
description: Use Kaito's transform and before lifecycle methods to handle CORS
---

# Handling CORS with Kaito

Kaito does not include any CORS handling out of the box. This is by design to keep the library lightweight and unopinionated. You can easily implement CORS handling in your server by using the [`before & transform`](/documentation/router-basics#request-lifecycle) lifecycle methods.
...
export const router = create({
	getContext,
	before: cors.before,
	transform: cors.transform,
});
```

In v4, `experimental_createCORSTransform()` returns an object with `.before` and `.transform` methods that you can pass directly to your router. It also provides methods for dynamic origin management:

- `cors.addOrigins(['https://new.example.com'])` - Add new allowed origins at runtime
- `cors.removeOrigins(['https://old.example.com'])` - Remove allowed origins at runtime
- `cors.setOrigins(['https://only-this.example.com'])` - Replace all allowed origins
- `cors.getOrigins()` - Get the current list of allowed origins
