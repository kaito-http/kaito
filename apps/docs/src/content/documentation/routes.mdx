...
Route query schemas should always take a string, or array of strings as the input. This is because query params are always strings. If you need to parse them into other types, you can do so inside the route handler.
...
Kaito schemas can be of any shape or size, including objects, booleans, numbers and literals. See the documentation for more details on the built-in schema system.
