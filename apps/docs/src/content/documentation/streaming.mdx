...
Kaito has a built-in utility for doing SSE, called `sse`. It returns a `KaitoSSEResponse` which Kaito automatically converts into a streaming response with the correct SSE headers.
