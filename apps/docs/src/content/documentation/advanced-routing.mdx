...
## Middleware with `.pipe()`

Instead of traditional middleware, Kaito offers `.pipe()` for a more predictable and type-safe approach to request processing.

### Basic Usage

`.pipe()` accepts a function that receives:

1. The current context
2. The router's parameters (if defined using `.params()`)
3. The `KaitoRequest` request object
4. The `KaitoHead` head object (setters for status & headers)

It should return the next context, which will be used for all subsequent routes:

```ts
const postsRouter = router
	.pipe(async (ctx, params, request, head) => {
...
You can chain multiple `.pipe()` calls, where each receives the context from the previous:
...
`.pipe()` enables powerful router composition patterns:
