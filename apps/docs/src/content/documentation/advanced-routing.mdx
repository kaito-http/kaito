...
The `.params()` method provides type-safe declaration of parameters that a router expects to receive when merged. It takes no runtime arguments — parameter names are specified purely as a string literal union type parameter, e.g. `.params<'userId'>()`. This is exclusively used with router merging - `.params()` is only effective when the router is merged somewhere that provides those parameters. All params are always strings extracted from the URL path.
