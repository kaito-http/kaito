...
```bash
bun i @kaito-http/core # Or use yarn, pnpm or npm
```

Kaito comes with its own built-in schema system for validation, so you do not need to install or use Zod or any other validation library. Kaito handles all validation natively.

## Concepts

Kaito revolves around a few basic concepts:
...
Kaito generates context on every single request. It is up to you to decide what to place inside your context, and it will be available in every single route. For example, you could include a method to get the current user session, or a database connection, or anything else you want.
