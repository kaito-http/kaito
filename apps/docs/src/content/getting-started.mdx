...
Kaito comes with its own built-in schema system for validation, so you do not need to install or use Zod or any other validation library. All validation is handled natively.
