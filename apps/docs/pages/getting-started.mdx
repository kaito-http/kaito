...
// Provide context to every procedure — our uptime and `req` & `res` objects
...
Secondly, we'll need to create a router, a server, and our first procedure.
...
const server = createServer({
	...
	// Define an `onError` handler in case something goes wrong inside of the route.
...
Awesome! So we now have a server that can be accessed at `http://localhost:8080/` that mounts a GET procedure to `/` which will respond with the server's uptime, and the time now. If you run this file and navigate to `http://localhost:8080/`, you'll see that result.
