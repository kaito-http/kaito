...
<Callout>
	Throwing a KaitoError will not call your `.onError` handler defined in your server. All other values thrown will!
</Callout>

```ts {8}
export const users = createRouter().add('GET', '/:id', async ({ctx, params}) => {
	const user = await ctx.db.users.findOne({
		where: {id: params.id},
	});

	// Client will receive this status code and error message
	if (!user) {
