...
```ts
const router = createRouter().add('GET', '/ping', async () => 'pong');
```
...
Routes are executed by the router. The router is responsible for parsing the request, validating the input, and executing the route. Route run methods can be async or sync. Throwing an error will call your onError handler defined in your server.
...
const router = createRouter().add('POST', '/echo', {
	query: {
