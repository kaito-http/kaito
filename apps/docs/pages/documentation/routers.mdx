...
```ts
const createPost = router().post("/posts", async ({ ctx, body, query, params }) => {
...
const usersRouter = router()
  .get("/posts", async ({ ctx, body, query, params }) => {
    // Some route that doesn't require the user.
    console.log(ctx.user); // <-- Undefined
  })
  .through(async (ctx) => {
    const session = ctx.getSession() // getSession is a function that provides the an object with a user property, or null
    if(!session) {
  ...
  .post("/posts", async ({ ctx, body, query, params }) => {
    // ctx.user is now a User object and can be accessed.
  })
```
...
const usersRouter = router()
  .get("/posts", async ({ ctx, body, query, params }) => {
    const posts = await getPosts(); // fictional function

    return {
      posts,
      self_user: ctx.user // => Undefined
    }
  })
  .through(async (ctx) => {
    const session = ctx.getSession() // getSession is a function that provides the an object with a user property, or null
  ...
  .post("/posts", async ({ ctx, body, query, params }) => {
    const post = await createPost(ctx.user.id);

    return post;
  })
  .through(async (ctx) => {
    // no need to check if user is null, because that last .through() threw a KaitoError
