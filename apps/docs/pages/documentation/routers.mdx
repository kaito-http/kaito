...
Routers can be created with the `createRouter` function exported by `@kaito-http/core`. Example below:
...
Kaito supports Router Merging, which is bringing one router's procedures into another, with a prefix. This is incredibly useful, for example, for having multiple versions of an API.
...
You can expect all type information to be carried over, and query URLs
